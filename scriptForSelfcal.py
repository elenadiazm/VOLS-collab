...
for i in range(0, len(my_vislist)):
    ...
    for my_submosaic in my_submosaics:
        ...
        mad = clean_stats['medabsdevmed'][0]

        print('MAD: '+ mad + ' Jy/beam')
        ...
        stats = imstat(imagename= my_imageFile +'.image.tt0', region=my_submosaicData['my_rms'][my_submosaic])

        rms = stats['rms'][0]

        rms_data = pd.DataFrame([[my_imageFile, rms]], columns=['observation', 'rms'])
        ...
        my_imageName = my_imageFile + '.fits' 

        print('Processing '+ my_imageName)

        # Run PyBDSF with the specified parameters

        img = bdsf.process_image(my_imageName,
                             adaptive_rms_box=True, # NOTE.- Maybe it is better to use the rms as the value that it was already obtained !! (rms)
                             thresh_isl=5.0,
                             thresh_pix=10.0)
    

        my_catalog = my_dir + 'data/' + my_imageFile  + '_10sigma_catalog.csv'
        ...
        print('You can check the catalog at 10sigma in ' + my_dir + 'data/' + my_imageFile + '_10sigma_catalog.csv')
        ...
        coords_deg = SkyCoord(ra=my_catalog_df['RA'] * u.degree, dec=my_catalog_df['DEC'] * u.degree, frame='icrs')
        ...
        error_coords = SkyCoord(ra=my_catalog_df['E_RA'] * u.degree, dec=my_catalog_df['E_DEC'] * u.degree, frame='icrs')
        ...
        for index, row in my_catalog_df.iterrows():
            ...
            for i, coord in enumerate(coords):
                ...
                if separation <= beam_radius * u.deg:
                    points_in_region.append(listobs.iloc[i]['Name'])

            my_catalog_df.at[index, 'pointings'] = points_in_region

        my_catalog_df.to_csv(my_catalog, index=False) # UPDATE THE CSV -- I AM NOT SURE IF IT IS REWRITTEN 
        ...
        bright_sources_df.to_csv(my_dir + 'data/' + my_imageFile  + '_bright_sources.csv', index=False) 

        print('You can check the bright sources catalog in ' + my_dir + 'data/' + my_imageFile + '_bright_sources.csv')

        print('::: VOLS ::: ... Working first with the bright sources')

        bright_sources_df.sort_values(by=['Peak_flux'], ascending=False) # Sorting from higher value to lower value of the peak flux

        for index,row in bright_sources_df.iterrows():

            my_pointings = str(row['pointings'])  # Get the pointings of each bright source
            pointings = set(my_pointings.split(",")) 
    
            new_pointings = pointings - selfcal_pointings  # Exclude existing pointings

            print('::: VOLS ::: ... The pointings ' + new_pointings + ' are going to be self-calibrated')
    
            my_fields = ",".join(new_pointings)

            my_fields_str = ",".join(bright_sources["pointings"].iloc[0]) # TO BE WRITTEN LIKE 'P8,P9,P10'
            my_fields_join = "".join(bright_sources["pointings"].iloc[0]) # TO BE WRITTEN LIKE P8P9P10

            selfcal_pointings.update(new_pointings)  # Add new pointings to the set !! NOTE.- This should be shown once the self-calibration is ready
            ...
            split(vis=my_visFile, 
              outputvis=my_visFile+'.'+my_fields_join, # NOTE.-  NEED TO CHECK HOW THIS IS WRITTEN
              field=my_fields_str,
              datacolumn = 'data')
            
            print('::: VOLS ::: ... The measurement set ' + my_visFile+'.'+my_fields_join, ' has been created')

            my_visFile_submosaic = my_visFile +'.'+my_fields_join
